...
if ! compare_cmd "run_dinitctl list" "initial.expected"; then
    ...
    error "'run_dinitctl list' didn't contain expected result!" "Check $IGR_OUTPUT/output/initial.actual"
fi
...
if ! compare_cmd "run_dinitctl --quiet reload boot" "output2.expected" err; then
    ...
    error "'run_dinitctl --quiet reload boot 2>&1' didn't contain expected result" "Check $IGR_OUTPUT/output/output2.expected"
fi
...
if ! compare_cmd "run_dinitctl --quiet list" "output3.expected"; then
    ...
    error "'run_dinitctl --quiet list' didn't contain expected result!" "Check $IGR_OUTPUT/output/output3.actual"
fi
