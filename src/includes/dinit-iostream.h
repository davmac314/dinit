...
/*
 * dinit-iostream is a specialized library providing input/output wrappers around POSIX I/O interfaces.
 ...
 * 4. "io_failbit" indicates that a POSIX I/O function failed, and the POSIX errno was set (ostream/istream).
 *
...
{
    public:
    iostream_system_err(const int posix_errno) : system_error(posix_errno, std::system_category())
    {
    }
};
...
class io_base
{
    ...
    // Set file descriptor
    // NOTE: Setting new fd will REPLACE current fd in the stream and It doesn't flush data to current fd
    // before replacing it. Also It may result in leaked file descriptor (because stream use its fd
    // to know what fd should be closed).
 ...
 * ------------------------------------------------------
 *
 * ostream through derived io_base class provides is_open() function which return true when current
 * file path is open or false when it's not.
 *
 * Error handling and Stream states
 * --------------------------------
 *
 * ostream states are: buffer_failbit and io_failbit.
 ...
 * 2. io_failbit is 0 by default and means there is something wrong with POSIX I/O calls, It's set to the
 *    POSIX errno in this case.
 *
 * NOTE: io_failbit is a bit but stream actually maintains a variable which contains the value of
 * errno in case of failure. This means io_failure() function will return 0 or errno and current_state()
 * will return true or false about io_failbit.
 *
 * Exception handling
 * ------------------
 *
...
class ostream : public io_base
{
    ...
    // io_error has different functionality, it will be set to POSIX errno which returned by POSIX
    ...
    void throw_exception_on(const unsigned states);

    public:
    ostream() noexcept
    {
        buf = std::unique_ptr<streambuf>(new(std::nothrow) streambuf);
        if (!buf) buffer_failed = true;
    }
    ...
    //
    // RETURN: true on success, false on failure cases.
    //
    // Non _nx variant may throw:
    // dio::iostream_internal_err on Buffer failure (buffer did not allocated at construction time)
    // dio::iostream_system_err on POSIX I/O failure
    bool close_nx() noexcept;
    bool close();

    // RETURN: current bits/vars in stream based on io_states enum.
    unsigned current_state() noexcept;

    // RETURN: true when there is no set bits/vars otherwise false.
    bool good() noexcept;

    // RETURN: true when buffer_failbit is set otherwise false.
    bool buffer_failure() noexcept;

    // RETURN: 0 or POSIX errno based on io_error value.
    ...
    //
    // RETURN: the size of buffer on full-flushed buffer, -1 on failure or not full-flushed buffer.
    //
    // Non _nx variant may throw:
    // dio::iostream_internal_err on Buffer failure (buffer did not allocated at construction time)
    // dio::iostream_system_err on POSIX I/O failure
    ssize_t flush_nx() noexcept;
    ssize_t flush();
    ...
    //
    // RETURN: true or false on failure cases or null/zero-length string.
    //
    // NOTE: write() does return true on null/zero-length.
    //
    // NOTE: write() does return true on partial write, Use write_buf() and check its return, if you
    // care about partial write.
    //
    ...
    //
    // RETURN: number of written-or-buffered characters or 0 on null/empty string or -1 on failure cases.
    //
    // Non _nx variants may throw:
    // dio::iostream_internal_err on Buffer failure (buffer did not allocated at construction time)
    // dio::iostream_system_err on POSIX I/O failure
    ...
    //      }
    //
    // RETURN: true when everything looks good otherwise false.
 ...
 * ------------------------------------------------------
 *
 * istream through derived io_base class provides is_open() function which return true when current
 * file path is open or false when it's not.
 *
 * Error handling and Stream states
 * --------------------------------
 *
 * istream states are: eofbit, buffer_failbit, string_failbit, and io_failbit.
 ...
 * 4. io_failbit is 0 by default and means there is something wrong from POSIX I/O calls, It's set
 *    to the POSIX errno in this case.
 *
 * NOTE: io_failbit is a bit but stream actually maintains a variable which contains the value of
 * errno in case of failure. This means io_failure() function will return 0 or errno and current_state()
 * will return true or false about io_failbit.
...
class istream : public io_base
{
    ...
    // RETURN: current bits in stream based on io_states enum.
    unsigned current_state() noexcept;

    // RETURN: true when there is no set bits/vars otherwise false.
    bool good() noexcept;

    // RETURN: true or false based on eofbit is set or not.
    bool eof() noexcept;

    // RETURN: true or false based on buffer_failbit is set or not.
    bool buffer_failure() noexcept;

    // RETURN: true or false based on string_failbit is set or not.
    bool string_failure() noexcept;

    // RETURN: 0 or POSIX errno based on io_error value.
    int io_failure() noexcept;

    // RETURN: failure bits/vars to false (or 0 about io_error).
    void clear() noexcept;

    // Reads and returns one character from file descriptor.
    //
    // RETURN: character as an integer or -1 on eofbit, buffer_failure, and posixio_failure.
    //
    // Non _nx variant may throw:
    ...
    //
    // RETURN: the amount of characters in the string or 0 on EOF or -1 on failure cases.
    //
    // Non _nx variant may throw:
    // dio::iostream_eof on EOF
    // dio::iostream_internal_err on Buffer failure (buffer did not allocated at construction time)
