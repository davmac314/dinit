...
DEFAULT_AUTO_RESTART=ALWAYS|ON_FAILURE|NEVER
    ALWAYS or NEVER auto-restart for services by default. ON_FAILURE will only auto-restart for
    services with non-zero exit code or services signalled with any signal other than SIGHUP,
    SIGINT, SIGUSR1, SIGUSR2 or SIGTERM. the default if unspecified is "ALWAYS".
