...
set service_management_commands start stop status restart wake release unpin unload reload enable disable

complete -f -c dinitctl -n "__fish_seen_subcommand_from $service_management_commands" -xa '(dinitctl list | string match -r -g "^.{10} ([\w-]+)")'
complete -f -c dinitctl -n "__fish_seen_subcommand_from $service_management_commands" -xa '(ls /etc/dinit.d/)'
